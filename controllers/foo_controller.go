...
import (
	...
	samplecontrollerv1alpha1 "github.com/yukihirop/sample-controller-kubebuilder/api/v1alpha1"
)
...
func (r *FooReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	/*
		...
		All client methods take a context (to allow for cancellation)
	...
	*/
	...
	/*
		...
		If there is a deployment which is owned by foo and it does't match foo.spec.deploymentName,
	...
	*/
	...
	r.Recorder.Eventf(&foo, corev1.EventTypeNormal, "Update", "Update foo.status.AvailableReplicas: %d", foo.Status.AvailableReplicas)
...
func (r *FooReconciler) cleanpOwnedResources(ctx context.Context, log logr.Logger, foo *samplecontrollerv1alpha1.Foo) error {
	...
	for _, deployment := range deployments.Items {
		...
		// Delete old deployment object which does't match foo.spec.deploymentName
